...
let truncateTextSimple = (length, text) => {
  text.length > length ? (!!text.slice ? text.slice(0, length) + "..." : "") : text;
}
...
function replaceAll(originalString, pattern, substitute) {
  return originalString.replace(new RegExp(escapeRegExp(pattern), 'g'), substitute);
}
  ...
  if (!!numforecasts) {
    ...
    return (
      <span className={`hidden sm:flex items-center justify-center text-gray-600 mt-2`}>
        ...
        {`Last updated: ${timestamp && !!timestamp.slice ? timestamp.slice(0, 10) : "unknown"}`}
      </span>
    )
  } else {
    return null
...
let forecastFooter = ({ stars, platform, numforecasts, qualityindicators, timestamp, showTimeStamp, expandFooterToFullWidth }) => {
  ...
  return (
    <div className={`grid grid-cols-3 ${expandFooterToFullWidth ? "justify-between" : ""} text-gray-500 mb-2 mt-1`}>
      <div className={`self-center col-span-1 ${getStarsColor(stars)} ${debuggingWithBackground ? "bg-red-200" : ""}`}>
        {getstars(stars)}
      </div>
      <div className={`${expandFooterToFullWidth ? "place-self-center" : "self-center"}  col-span-1 font-bold ${debuggingWithBackground ? "bg-red-100" : ""}`}>
        {platform
      ...
      <div className={`${expandFooterToFullWidth ? "justify-self-end mr-4" : "justify-self-center"} col-span-1 ${debuggingWithBackground ? "bg-red-100" : ""}`}>
  ...
  return (
    ...
    >
      <div className="flex-grow">
        <div className={`text-gray-800 ${opacityFromScore(score)} text-lg mb-2 font-medium justify-self-start`}>
          <div
            className={`${showIdToggle ? `    ` : "hidden"} flex items-center justify-center mt-2 mb-3 text-sm bg-transparent  py-4 px-4 border rounded mt-5 p-10 text-center mb-2 mr-10 ml-10 hover:bg-blue-300 text-blue-400 text-blue-700 ${isJustCopiedSignalVisible ? " hover:text-white hover:border-blue-700 border-blue-400 hover:bg-blue-700" : "border-blue-400 hover:text-white hover:border-transparent"}`}  id="toggle"
            onClick={(e) => {
              e.preventDefault();
        ...
        {(options.length == 2 && (options[0].name == "Yes" || options[0].name == "No")) && (
          <div>
            <div className="grid  mb-5 mt-4 mb-5 grid-cols-1 sm:grid-rows-1">
              <div
                className="flex-grow col-span-1 w-full items-center justify-center"
              >
                <span
      ...
      <div className={`${opacityFromScore(score)} w-full`}>{forecastFooter({ stars: qualityindicators.stars, platform: author || platform, numforecasts: qualityindicators.numforecasts, qualityindicators, timestamp, showTimeStamp, expandFooterToFullWidth })}
      </div>
...
export default function displayForecasts({ results, numDisplay, showIdToggle }) {
  return (
    !!results && !!results.slice ?
      results.slice(0, numDisplay).map((fuseSearchResult) => {
        ...
        */
        let display = displayForecast({ ...fuseSearchResult.item, score: fuseSearchResult.score, showTimeStamp: false, expandFooterToFullWidth: false, showIdToggle })
        return display;
      }) : (<></>)
  );
}
