...
let numerateForecasts = ({numforecasts, timestamp, showTimeStamp, qualityindicators, platform}) => {
  return(<div className="grid grid-cols-1">
      ...
      {Object.entries(formatQualityIndicators(qualityindicators))
        .map(entry => {
          return(
          <div className="flex col-span-1 row-span-1 mr-2">
            <span>{`${entry[0]}: `}</span>&nbsp;
            <span className="font-bold">
              {`${getCurrencySymbolIfNeeded({indicator: entry[0], platform})}${formatNumber(entry[1])}`}
            </span>
          </div>
          )
        })
...
let forecastFooter = ({stars, platform, numforecasts, qualityindicators, timestamp, showTimeStamp}) => {
  ...
  return (
    <div className="grid grid-cols-3 grid-rows-1 items-center content-center text-gray-500">
      ...
      <div className="flex col-start-3 col-end-3 ml-2">
        {numerateForecasts({numforecasts, timestamp, showTimeStamp, qualityindicators, platform})}
      </div>
    </div>
  );
};
        ...
        {(options.length == 2 && (options[0].name == "Yes" || options[0].name == "No")) && (
          <div>
            ...
            <div className="grid mb-5 mt-4 mb-5 grid-cols-1 sm:grid-rows-1">
              ...
              <div className={`hidden sm:${showTimeStamp  && !checkIfDisplayTimeStampAtBottom(qualityindicators) ? "flex" : "hidden"} ${opacityFromScore(score)} row-end-2 col-start-2 col-end-2 row-start-1 row-end-1 col-span-1 items-center justify-center text-gray-600 ml-2 mr-2 `}>
