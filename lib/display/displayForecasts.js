    ...
    >
      <div className="flex-grow">
        <div className={`text-gray-800 ${opacityFromScore(score)} text-lg mb-2 font-medium justify-self-start`}>
          <div className={`${showId ? "flex items-center justify-center mt-2 mb-3 text-sm bg-transparent hover:bg-blue-300 text-blue-700 hover:text-white py-4 px-4 border border-blue-500 hover:border-transparent rounded mt-5 p-10 text-center mb-2 mr-10 ml-10 items-center justify-center" : "hidden"}`}
            onClick={(e) => {
              e.preventDefault();
            ...
            {`[${id}]`}<FaRegClipboard className="ml-3"/>
          </div>
          {title.replace("</a>", "")}
        </div>
...
export default function displayForecasts({ results, numDisplay, showId }) {
  return (
    !!results && !!results.slice ?
      results.slice(0, numDisplay).map((fuseSearchResult) => {
        ...
        */
        let display = displayForecast({ ...fuseSearchResult.item, score: fuseSearchResult.score, showTimeStamp: false, expandFooterToFullWidth: false, showId })
        return display;
      }) : (<></>)
  );
}
