...
let truncateText = (length, text) => {
  ...
  let truncatedText = text.slice(0, lastIndex)+ (lastLetter != "." ? "..." : "..")
...
let displayQualityIndicators = ({numforecasts, timestamp, showTimeStamp, qualityindicators, platform}) => { 
  ...
  return(<div className="text-sm">
      ...
      {Object.entries(formatQualityIndicators(qualityindicators))
        .map(entry => {
          return(
          <div className="col-span-1 row-span-1" key={entry[0]}>
            <span>{`${entry[0]}:`}</span>&nbsp;
...
let forecastFooter = ({stars, platform, numforecasts, qualityindicators, timestamp, showTimeStamp, expandFooterToFullWidth}) => {
  ...
  /*
  return (
    <div className="flex grid-cols-3 gap-2 items-center align-center text-gray-500 w-full mb-2">
      <div className={`flex col-span-1 items-center justify-center text-sm ${getStarsColor(stars)} ${debuggingWithBackground?  "bg-red-100" : ""}`}>
        {getstars(stars)}
      </div>
      <div className={`flex col-span-1 items-center justify-center align-center text-sm ${debuggingWithBackground? "bg-red-100" : ""}`}>
          {platform
            .replace("Good Judgment Open", "GJOpen")
            .replace("OpenPhilanthropy", "OpenPhil")
            .replace("AstralCodexTen", "ACX")
      ...
      <div className={`flex col-span-1 items-center align-center justify-center  ${debuggingWithBackground? "bg-red-100" : ""}`}>
        {displayQualityIndicators({numforecasts, timestamp, showTimeStamp, qualityindicators, platform})}
      </div>
    </div>
  );
...
export function displayForecast({
  ...
  return(
    ...
    >
      ...
      <div className={`${opacityFromScore(score)} w-full`}>{forecastFooter({stars: qualityindicators.stars, platform: author || platform, numforecasts: qualityindicators.numforecasts,  qualityindicators, timestamp, showTimeStamp, expandFooterToFullWidth})}</div>
    </a>
  );
}
...
export default function displayForecasts({results, numDisplay}) {
  return (
    ...
    results.slice(0, numDisplay).map((fuseSearchResult) => {
      ...
      */
      let display = displayForecast({ ...fuseSearchResult.item, score: fuseSearchResult.score, showTimeStamp: false, expandFooterToFullWidth: false})
      return display;
    })
  );
}
