...
let forecastFooter = ({stars, platform, numforecasts, qualityindicators, timestamp, showTimeStamp}) => {
  ...
  return (
    <div className="flex grid-cols-3 items-center justify-start gap-8 text-gray-500 w-full mb-2 mt-1">
      <div className={` ${getStarsColor(stars)} ${debuggingWithBackground?  "bg-red-200" : ""}`}>
        {getstars(stars)}
      </div>
      <div className={`font-bold ${debuggingWithBackground? "bg-red-100" : ""}`}>
          {platform
      ...
      <div className={`  ${debuggingWithBackground? "bg-red-100" : ""}`}>
