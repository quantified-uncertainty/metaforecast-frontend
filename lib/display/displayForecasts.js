...
let forecastFooter = ({stars, platform, numforecasts, qualityindicators, timestamp, showTimeStamp}) => {
  ...
  return (
    <div className="grid grid-cols-3 text-gray-500 mb-2 mt-1">
      <div className={`self-center col-span-1 ${getStarsColor(stars)} ${debuggingWithBackground?  "bg-red-200" : ""}`}>
        {getstars(stars)}
      </div>
      <div className={`self-center col-span-1 font-bold ${debuggingWithBackground? "bg-red-100" : ""}`}>
          {platform
      ...
      <div className={`justify-self-center col-span-1 ${debuggingWithBackground? "bg-red-100" : ""}`}>
