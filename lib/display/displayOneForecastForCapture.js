...
let generateSource = (result, imgSrc, displayCapture) => {
  ...
  if (result && imgSrc && displayCapture) {
    return (
      <div className="grid">
...
let generateMetaculusSource = (result, displayCapture) => {
  const [htmlButtonStatus, setHtmlButtonStatus] = useState("Copy HTML");
  let handleHtmlButton = () => {
    setHtmlButtonStatus("Copied")
    let newtimeoutId = setTimeout(async () => {
  ...
  if (result && displayCapture && result.item.platform == "Metaculus") {
    return (
      <div className="grid">
...
export default function displayOneForecast({result, displayCapture, setDisplayCapture}) {
  const containerRef = useRef(null);
  //const imgRef = useRef(null);
  const [imgSrc, setImgSrc] = useState("");
  const [mainButtonStatus, setMainButtonStatus] = useState("Capture image and generate code");
  ...
  /*
  let onInitialLoadForSecretEmbed = () => {
    setTimeout(()=>{
      if(!clickedAlreadyBool){
        setClickAlreadyBool(true)
  ...
  */
  function generateCaptureButton(result, onCaptureButtonClick) {
    if (result) {
      return (
        <button onClick={() => onCaptureButtonClick()} className="bg-blue-500 cursor-pointer px-5 py-4 bg-white rounded-md shadow text-white hover:bg-blue-600 active:bg-gray-700">
          {mainButtonStatus}
        </button>
      )
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full justify-center">
      ...
      <div className="flex col-span-1 items-center justify-center">
        <img src={imgSrc} className={displayCapture ? "" : "hidden"}></img>
      </div>
      <div className="flex col-span-1 items-center justify-center">
        <div>
          {generateSource(result, imgSrc, displayCapture)}
        </div>
      </div>
      ...
      <div className="flex col-span-1 items-center justify-center">
        <div>
          {generateMetaculusSource(result, displayCapture)}
        </div>
      </div>
      <br></br>
    </div>
