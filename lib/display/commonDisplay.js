...
export default function CommonDisplay({
    ...
    const [whichResultToDisplayAndCapture, setWhichResultToDisplayAndCapture] = useState(0);
    ...
    async function executeSearchOrAnswerWithDefaultResults(queryData) {
        ...
        let filterManually = (queryData, results) => {
            if (
                ...
                let forecastingPlatforms = queryData.forecastingPlatforms.map(platformObj => platformObj.value)
    ...
    return (
        ...
        >
            <label className="mb-4 mt-4 flex flex-row justify-center items-center" key={"common-1"}>
                <div className={`w-10/12 mb-2 ${hasSearchbar ? "" : "hidden"}`} key={"common-1-1"}>
                    <Form
            ...
            >
                ...
                >
                    ...
                    <button
                        className="block col-start-1 col-end-4 md:col-start-2 md:col-end-3 md:row-start-4 md:row-end-4 lg:col-start-2 lg:col-end-3 lg:row-start-4 lg:row-end-4 bg-transparent hover:bg-blue-300 text-blue-700 hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded mt-5 p-10 text-center mb-2 mr-10 ml-10 items-center justify-center"
            ...
            >
                <p
                    className={`mt-4 mb-4 ${(!results || ((results.length != 0) && queryParameters.numDisplay < results.length))
                        ? ""
                        : "hidden"
                        }`}
                    key={"common-4-1"}
                ...
                >
                    ...
                    <span
                        className={`cursor-pointer text-blue-800 ${!results ? "hidden" : ""}`}
                        onClick={() => {
                            setQueryParameters({
                                ...queryParameters,
