...
let noExactMatch = (queryString, result) => {
  ...
  return !(title.includes(queryString) || description.includes(queryString) || optionsstringforsearch.includes(queryString))
}
...
export default async function searchWithAlgolia({ queryString, hitsPerPage, starsThreshold, filterByPlatforms, forecastsThreshold }) {
  ...
  if ((!results || results.length == 0) && !recursionError.includes(queryString.toLowerCase())) {
    results = [{
    ...
    results = [{
      ...
      "platform": "Metaforecast",
      "description": "Fatal error: Too much recursion. Click to proceed anyways",
      "options": [
        {
          "name": "Yes",
          "probability": 0.995,
  ...
  } else if ((queryString && queryString.split(" ").length == 1) && noExactMatch(queryString, results[0])) {
    results.unshift(
      {
