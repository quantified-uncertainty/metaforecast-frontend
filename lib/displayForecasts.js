...
let numerateForecasts = (number, timestamp, showTimeStamp) => {
  if (!!number) {
    ...
    return(
      <span className={`hidden sm:flex items-center justify-center text-gray-600 ${true ? "" : "hidden"}`}>
...
export function displayForecast({
  ...
  return(
    ...
    >
      <div className="flex-grow">
        ...
        {(options.length == 2 && (options[0].name == "Yes" || options[0].name == "No")) && (
          <div>
            ...
            <div className="grid mb-5 mt-4 mb-5 grid-cols-1 sm:grid-rows-1">
              ...
              <div className={`hidden sm:flex row-end-2 col-start-2 col-end-2 row-start-1 row-end-1 col-span-1 items-center justify-center text-gray-600 ml-2 mr-2 ${showTimeStamp && qualityindicators.numforecasts ? "" : "hidden"}`}>
                <svg className="ml-4 mr-1 mt-1" height="10" width="16">
                  <circle cx="4" cy="4" r="4" fill="rgb(29, 78, 216)" />
                </svg>
                {`Last updated: ${timestamp ? timestamp.slice(0,10) : "unknown" }`}
        ...
        {( options.length != 2 || (options[0].name != "Yes" && options[0].name != "No")) && (
          <>
            ...
            <div className={`hidden sm:flex col-start-2 col-end-2 row-start-1 row-end-1 text-gray-600 mt-3 ${showTimeStamp && qualityindicators.numforecasts ? "" : "hidden"}`}>
      ...
      <div className={`flex sm:hidden items-center justify-center mt-2 mb-4 text-gray-600 ${true ? "" : "hidden"}`}>
