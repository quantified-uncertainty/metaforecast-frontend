...
let numerateForecasts = (number, timestamp, showTimeStamp) => {
  if (!!number) {
    ...
    return(
      <span className={`hidden sm:flex items-center justify-center text-gray-600`}>
        <svg className="ml-4 mr-1 mt-1" height="10" width="16">
          <circle cx="4" cy="4" r="4" fill="rgb(29, 78, 216)" />
        </svg>
        {`Last updated1: ${timestamp ? timestamp.slice(0,10) : "unknown" }`}
      </span>
    )
  }
};
...
let forecastFooter = (stars, platform, numforecasts, timestamp, showTimeStamp) => {
  ...
  return (
    <div className="grid grid-cols-3 grid-rows-1 items-center content-center text-gray-500">
      <div className="flex col-start-1 col-end-1 text-yellow-400 opacity-80">
        {getstars(stars)}
      </div>
      <div className="flex col-start-2 col-end-2 font-bold text-sm">
        {platform.replace("Good Judgment Open", "GJOpen").replace(/ /g, "\u00a0")  // replaceAll(" ", "\u00a0")
        }
      </div>
      <div className="flex col-start-3 col-end-3 ">
...
export function displayForecast({
  ...
  return(
    ...
    >
      <div className="flex-grow">
        ...
        {(options.length == 2 && (options[0].name == "Yes" || options[0].name == "No")) && (
          <div>
            ...
            <div className="grid mb-5 mt-4 mb-5 grid-cols-1 sm:grid-rows-1">
              ...
              <div className={`hidden sm:${showTimeStamp && qualityindicators.numforecasts ? "" : "hidden"} row-end-2 col-start-2 col-end-2 row-start-1 row-end-1 col-span-1 items-center justify-center text-gray-600 ml-2 mr-2 `}>
                <svg className="ml-4 mr-1 mt-1" height="10" width="16">
                  <circle cx="4" cy="4" r="4" fill="rgb(29, 78, 216)" />
                </svg>
                {`Last updated2: ${timestamp ? timestamp.slice(0,10) : "unknown" }`}
              </div>
            </div>
          </div>
        )}
        {( options.length != 2 || (options[0].name != "Yes" && options[0].name != "No")) && (
          <>
            ...
            <div className={`hidden sm:${showTimeStamp && qualityindicators.numforecasts ? "flex" : "hidden"} col-start-2 col-end-2 row-start-1 row-end-1 text-gray-600 mt-3`}>
              <svg className="ml-6 mr-1 mt-2" height="10" width="16">
                <circle cx="4" cy="4" r="4" fill="rgb(29, 78, 216)" />
              </svg>
              {`Last updated3: ${timestamp ? timestamp.slice(0,10) : "unknown" }`}
            </div>
          </>
        )}
      ...
      <div className={`flex sm:hidden items-center justify-center mt-2 mb-4 text-gray-600`}>
        <svg className="" height="10" width="16">
          <circle cx="4" cy="4" r="4" fill="rgb(29, 78, 216)" />
        </svg>
        {`Last updated4: ${timestamp ? timestamp.slice(0,10) : "unknown" }`}
      </div>
      <div className={`${opacityFromScore(score)} w-full`}>{forecastFooter(qualityindicators.stars, author || platform, qualityindicators.numforecasts,  timestamp, showTimeStamp)}</div>
    </a>
  );
}
